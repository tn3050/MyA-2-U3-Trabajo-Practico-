...
void Game::UpdatePhysics()
{
	...
	if (cuerpoSeleccionado) {
		...
		cuerpoSeleccionado->SetTransform(b2Vec2(worldPos.x, worldPos.y), cuerpoSeleccionado->GetAngle());
		cuerpoSeleccionado->SetLinearVelocity(b2Vec2_zero);
		cuerpoSeleccionado->SetAwake(true);
	}
				...
				public:
					...
					bool ReportFixture(b2Fixture* f) override {
						if (f->GetBody()->GetType() == b2_dynamicBody && f->TestPoint(point)) {
							fixture = f;
							return false; // Ya encontramos uno
						}
						return true;
					}
				};

				QueryCallback callback(mouseWorld);
				phyWorld->QueryAABB(&callback, aabb);

				// 👇 AQUÍ va lo que preguntaste:
				if (callback.fixture) {
					cuerpoSeleccionado = callback.fixture->GetBody(); // <- seleccionamos el cuerpo
				}
			}
			break;
...
void Game::InitPhysics()
{
	...
	Box2DHelper::CreateDistanceJoint(phyWorld, circuloDer, circuloDer->GetWorldCenter(),
		circuloIzq, circuloIzq->GetWorldCenter(), 20.0f, 0.1f, 1.0f);
}
